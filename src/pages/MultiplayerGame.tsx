...
export default function MultiplayerGame() {
  ...
  // Socket events
  useEffect(() => {
    const unsubGuess = on("game:guess_result", (_data: any) => {
      // Update scores
    });

    const unsubRoundResults = on("game:round_results", (data: any) => {
  ...
  const handleSubmitGuess = useCallback(() => {
    if (!guessLocation || !lobbyStore.lobbyCode) return;

    // Calculate distance and score (simplified)
    const mockDistance = Math.random() * 10000000;
    const mockScore = Math.round(5000 * Math.exp(-3 * mockDistance / 15000000));

    setTotalScore((s) => s + mockScore);
    setLastGuess({
      score: mockScore,
    ...
    setStatus("round_end");
  }, [guessLocation, lobbyStore.lobbyCode, roundNumber]);
  ...
  const handleNextRound = () => {
    ...
    setStatus("playing");
    setTimeLeft(lobbyStore.settings.roundTime);
    setLastGuess(null);
  };

  const minutes = Math.floor(timeLeft / 60);
  const seconds = timeLeft % 60;
  const timeColor = timeLeft <= 10 ? "text-red-500" : timeLeft <= 30 ? "text-yellow-500" : "text-white";
    ...
    <div className="h-screen flex flex-col">
      ...
      <div className="flex-1 flex flex-col lg:flex-row">
        ...
        <div className="flex-1 bg-[#1A1D24] relative flex items-center justify-center min-h-[200px] overflow-hidden">
          <PanZoomImage 
            src={`https://loremflickr.com/800/600/city,landmark/all?lock=${roundNumber}`} 
          ...
          <div className="absolute bottom-4 left-4 bg-black/75 backdrop-blur-md px-4 py-2 rounded-xl text-xs border border-gray-700/50 pointer-events-none select-none">
            <span className="text-[#E6C200] font-bold uppercase tracking-wider">Round {roundNumber}</span>
          </div>
        </div>
        ...
        <div className="flex-1 relative min-h-[300px]">
          ...
          >
            ...
            {status === "round_end" && lastGuess && (
              <>
                <Marker position={[lastGuess.actualLocation?.lat || 0, lastGuess.actualLocation?.lng || 0]} icon={actualIcon} />
                {guessLocation && (
                  <Polyline
                    positions={[
                      [guessLocation.lat, guessLocation.lng],
